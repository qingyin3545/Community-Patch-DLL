...
bool CvTraitEntry::CacheResults(Database::Results& kResults, CvDatabaseUtility& kUtility)
{
	...
	//EraCoastCityYieldChanges
	{
		...
		if(pResults == NULL)
		{
			pResults = kUtility.PrepareResults(strKey, "select Era.ID as EraID, Yields.ID as YieldID, Yield from Trait_EraCoastCityYieldChanges inner join Eras on Eras.Type = EraType inner join Yields on Yields.Type = YieldType where TraitType = ?");
		}
